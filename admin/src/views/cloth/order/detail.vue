<template>
    <sc-page-header
        :title="id ? '编辑订单' : '新增订单'"
        icon="el-icon-burger"
        :descripttion="id ? form.code : '新增订单'"
    >
        <template #main>
                ...
                <el-col :lg="{ span: 20, offset: 3 }">
                    ...
                    >
                        ...
                        <el-form-item
                            prop="contains"
                            label="数量"
                            v-if="form.cloth.sizes.length > 0 && form.cloth.colors.length > 0"
                        >
                            <table class="contains">
                                <thead>
                                    <tr>
                                        <th>颜色</th>
                                        <th v-for="size in form.cloth.sizes" :key="size">
                                            {{ size }}
                                        </th>
                                        <th>合计</th>
                                    </tr>
                        ...
                        <el-form-item label="物料" prop="consumption">
                            <table>
                                <thead>
                                    ...
                                    <th>全部尺码</th>
                                    <th v-for="color in form.cloth.colors" :key="color">
                                        {{ color }}
                                    </th>
                                    <th>操作</th>
                                </thead>
                                    ...
                                    <tr v-for="(item, index) in form.consumption" :key="index">
                                        ...
                                        <td>
                                            <el-button
                                                ...
                                                plain
                                                icon="el-icon-delete"
                                                @click="form.consumption.splice(index, 1)"
                                            ></el-button>
                                        </td>
                                    </tr>
                                </tbody>
...
export default {
    ...
    watch: {
        ...
        'form.cloth.colors': {
            handler: function (val, oldVal) {
                const oldContains = this.form.contains
                const contains = []
                val.forEach((color) => {
    ...
    methods: {
        ...
        addConsumptionBtn() {
            const consumption = JSON.parse(JSON.stringify(this.addConsumption))
            this.form.consumption.push(consumption)
        },
        async getDetail() {
            const res = await this.$API.order.order.show.get({ id: this.id })
            if (res.code == 200) {
        ...
        handleContainsChange(val) {
            ...
            const contains = this.form.contains
            const containsMap = {}
            contains.forEach((item) => {
        ...
        handleClothCodeChange(val) {
            const cloth = this.cloths.find((item) => item.code == val)
            if (cloth) {
        ...
        handleClothNameChange(val) {
            const cloth = this.cloths.find((item) => item.name == val)
            if (cloth) {
                this.form.cloth = cloth
                this.form.procedure = cloth.procedures
            }
        ...
        async querySearchCloth(query) {
            ...
            const res = await this.$API.order.style.list.get(params)
            if (res.code == 200) {
        ...
        querySearchCustomer(queryString, cb) {
            const results = queryString
                ? this.customers.filter(this.createFilterCustomer(queryString))
                : this.customers
            cb(results)
        },
        ...
        querySearchSalesman(queryString, cb) {
            const results = queryString
                ? this.salesmans.filter(this.createFilter(queryString))
                : this.salesmans
            cb(results)
        },
        querySearchOrderType(queryString, cb) {
            const results = queryString
                ? this.order_types.filter(this.createFilter(queryString))
                : this.order_types
            cb(results)
        },
        ...
        async getInfo() {
            ...
            const res = await this.$API.basic.dic.query.get(params)
            if (res.code == 200) {
            ...
            const customer = await this.$API.basic.customer.list.get({ page: 1, pageSize: 1000 })
